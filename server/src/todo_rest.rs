...
impl SQLWrapper {
 ...
 //"{\"dc7b5396c805c06f0ac60390322e4142a0bceb35\": {\"iv\": [-103, 37, -19, -3, 47, -121, -31, 79, 1, 54, 94, 56, -107, 100, -87, 104],\"salt\" : [1, 3, 3, 7, 4, 2, 0, 6, 9],\"algo\": \"AES/CBC/PKCS5Padding\"}}".to_string()
        let mut stmt = self.conn.prepare(format!("SELECT  json from encrstate where name_hash == \"{}\"", "fridolin").as_str()).unwrap();
...
pub fn est_database_conn() -> Connection {
    ...
    conn.execute("INSERT into enrstate (name_hash, json) values (\"fridolin\", \"{allah}\";", []);
    conn
}